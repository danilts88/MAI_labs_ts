...
int main(int argc, char*argv[]) {
    ...
    if ((c = getStr()) != NULL) {
        ...
        printf("The number system was not entered or an incorrect format was entered\n");
        return 0;
    }
    ...
    while (( c = getStr()) != NULL) {
        if (strcmp(c, "Stop") == 0 || strcmp(c, "stop") == 0) {
            ...
            if (checkBaseAndNum(base, c) == -1) {
                printf("The number does not correspond to the entered number system\n");
                free(c);
                return 0;
            }
            int value = toInt(c);
    ...
    if (c == NULL) {
        printf("The input format is incorrect or you forgot to enter the stop command\n");
        return 0;
    }
    char res[256];
    int len = 0;
