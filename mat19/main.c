...
int main(int argc, char*argv[]) {
    if (argc == 3) {
        if (Check(argv[1]) != OK || Check(argv[2]) != OK) {
            printf("The input format is incorrect or the number is too large/small, enter the number\n");
            return 0;
        }
        int a = atoi(argv[1]);
        int b = atoi(argv[2]);
        if (a > b) {
            printf("Incorrect input format, the first number is the minimum, the second is the maximum\n");
            return 0;
        }
        int mas[SIZE];
