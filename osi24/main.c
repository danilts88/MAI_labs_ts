...
int main(int argc, char *argv[]) {
    if (argc != 3) {
        printf("Pass the path to the file and the search string\n");
        return 0;
    }
    FILE *in;
    ...
    if (ans == OK) {
        int i = 0;
        if (!(names[i])) {
            printf("There is no specified line in any file\n");
            fclose(in);
            return 0;
        }
        while (names[i]) {
        ...
        printf("Too many files have been transferred or their names are too long\n");
        fclose(in);
        return 0;
    }
    else if (ans == ERROR_FILE) {
        printf("One of the transferred files could not be opened or the format is not txt\n");
        fclose(in);
        return 0;
    }
    else {
        printf("Error in the process/shared memory\n");
        fclose(in);
        return 0;
    }
    return 0;
