...
int checkArgs(char flag, int count, char*argv[]) { //проверяем аргументы и соответствие их кол-ва флагу
    if ((flag=='q' && count!=6) || (flag=='m'  && count!=4) || (flag=='t' && count!=6))
        return 0;
...
int main(int argc, char*argv[]) {
    if(((argc<4 || argc>6) || (strlen(argv[1])!=2)) || ((argv[1][1]!='q') && (argv[1][1]!='m') && (argv[1][1]!='t') ) || ((argv[1][0]!='-') && (argv[1][0]!='/'))) {
        ...
        printf("Possible flags: -q, -m, -t \n");
        printf("The symbol in front of the flag: - or /\n");
        return 0;
    } //проверка на корректность ввода

    if(checkArgs(argv[1][1], argc, argv)==0) {
        printf("The number of arguments does not match the flag or the numbers are entered incorrectly\n");
        return 0;
    }
    ...
    if (argv[1][1]=='t') {
        ...
        flagT(num1, num2, num3, eps)==1 ? printf("Possible\n") : printf("Impossible\n");
        return 0;
    }
    ...
    if (argv[1][1] == 'q') {
        ...
        for(int i=0; i<countPerest; i++) {
            ...
            printf("the answer =");
            if (masAnswers[i][0]==0) printf("There are no roots\n");
            else if (masAnswers[i][0]==2) printf("%f, %f\n", masAnswers[i][1], masAnswers[i][2]);
            else if (masAnswers[i][0]==3) printf("Any numbers\n");
            else printf("%f\n", masAnswers[i][1]);
        }
    }
