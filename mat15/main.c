...
int main(int argc, char*argv[]) {
    ...
    if (isInInt(argv[2]) == 0 || isInt(argv[2],1) == 0 || isInt(argv[1], 0) == 0){
        printf("Overflow or incorrect input format\n");
        return 0;
    }

    int x = atoi(argv[2]);
    //1)
    printf("For the first option, the answer is: %d\n", firstTask(x, eps));

    //2)
    printf("For the second option, the answer is: %f\n", secondTask(x, eps));

    //3)
    printf("For the third option, the answer is: %f\n", thirdTask(x, eps));

    //4
    printf("For the fourth option, the answer is: %f\n", fourthTask(x, eps));
